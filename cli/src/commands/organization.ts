...
export async function selectOrganization(): Promise<OrganizationSelectionResult> {
	...
	try {
		...
		if (customers.length === 0) {
			...
			console.log(
				chalk.gray("Please make sure you have the correct permissions.\n"),
			);
			process.exit(1);
		}
...
async function handleNonContributorFlow(
	...
	console.log(
		chalk.gray(
			"To create mittwald extensions, the organization needs contributor status.\n",
		),
	);
	...
	switch (action) {
		...
		case "exit":
			...
			console.log(
				chalk.white("To become a contributor, please follow the guide at:"),
			);
			console.log(
...
async function submitInterestFlow(
	...
	console.log(chalk.white(`Organization: ${customer.name}`));
	console.log(
		chalk.gray("This will submit your interest to become a contributor.\n"),
	);
	...
	try {
		...
		console.log(chalk.bold.white("\n✓ Interest Submitted\n"));
		console.log(
			chalk.white("Your interest to become a contributor has been submitted."),
		);
		console.log(
			chalk.gray("You will be notified when your application is reviewed.\n"),
		);
