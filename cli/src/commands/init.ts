...
export async function init(): Promise<void> {
	try {
		...
		console.log(
			chalk.white(
				"First, let's select which organization you want to create an extension for.",
			),
		);

		const { selectedCustomerId, isContributor: hasContributorAccess } =
		...
		console.log(
			chalk.white("Choose the context where your extension will be available:"),
		);

		const { extensionContext } = await inquirer.prompt<
			Pick<ProjectConfig, "extensionContext">
		>([
			{
				type: "list",
		...
		if (extensionContext === "project") {
			...
			try {
				...
				if (projects.length === 0) {
					...
					console.log(
						chalk.gray(
							"You need at least one project to create a project-level extension.",
						),
					);
					process.exit(1);
				}
...
`;
		...
		// Step 5: Extension Configuration (we already confirmed contributor status)
		{
			...
			console.log(
				chalk.white("1. Create Extension in mStudio Contributor UI:"),
			);
			console.log(
				chalk.gray('   • Navigate to "Entwicklung" in your organization'),
			);
			console.log(
				chalk.gray("   • Create a new extension and copy the EXTENSION_ID"),
			);
			console.log(
				chalk.gray("   • Set extension context to: ") +
					chalk.bold(extensionContext),
			);
			console.log(
				chalk.gray("   • Set anchor URL to: ") + chalk.bold(anchorUrl),
			);
			...
			const { extensionId } = await inquirer.prompt<
				Pick<ProjectConfig, "extensionId">
			>([
				{
					type: "input",
					name: "extensionId",
			...
			console.log(
				chalk.white(
					`   • Extension Secret: ${chalk.green("Auto-generated ✓")}`,
				),
			);
			console.log(
				chalk.white(`   • Context: ${chalk.green(`${extensionContext} ✓`)}`),
			);
			console.log(
				chalk.white(`   • Anchor URL: ${chalk.green(`${anchorUrl} ✓`)}`),
			);
			console.log(
				chalk.white(`   • Scopes: ${chalk.green("Empty (as requested) ✓")}`),
			);
			...
			console.log(
				chalk.white("   • Deploy to public URL (ngrok, cloudflared, etc.)"),
			);
...
`;
				...
				await fs.writeFile(envPath, envContent);
				console.log(
					chalk.green("✓ Extension configuration saved to .env file"),
				);
			} catch {
				console.log(
					chalk.bold.white(
						"⚠️  Could not save extension configuration to .env file",
					),
				);
				console.log(
