...
`),
	...
	console.log(
		chalk.white("  init           ") +
			chalk.gray("Initialize a new mittwald extension project (default)"),
	);
	console.log(
		chalk.white("  auth:login     ") +
			chalk.gray("Authenticate with mittwald OAuth"),
	);
	console.log(
		chalk.white("  auth:logout    ") +
			chalk.gray("Clear authentication tokens"),
	);
	console.log(
		chalk.white("  auth:status    ") +
	...
	console.log(
		chalk.gray("  • Authentication is required before creating projects"),
	);
	console.log(
		chalk.gray("  • OAuth callback uses port 52847 (must be available)"),
	);
	console.log(
		chalk.gray("  • Configuration is stored in ~/.mittvibes/config.json\n"),
	);
}
